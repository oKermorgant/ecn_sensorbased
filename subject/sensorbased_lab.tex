...
\subsection{Structure of the \okt{ecn_sensorbased} ROS package}
...
The simulation can be launched with: \okt{roslaunch ecn_sensorbased coppelia_sim.launch}. It has to be run manually by clicking the \okt{Play} button.
In case the robot goes wild, you can hit the \okt{Stop} button to reset the simulation.
...
The gain related to the constraints is expressed in the variable \okt{lc} and is set to 2.
...
The main methods of the \okt{PioneerCam} class can be seen in the \okt{pioneer.h} file:
\begin{itemize}
 \item Getters for robot model or measurements:
 \begin{itemize}
  \item \okt{double robot.radius()}, \okt{robot.base()}, \okt{robot.wmax()} : kinematic model.
  \item \okt{vpFeaturePoint robot.imagePoint()}: get the measured center of gravity of the sphere in the image, in normalized coordinates.
  \item \okt{vpColVector robot.camLimits()}: get the upper bounds for $(x,y)$. Lower bounds are the opposite.
  \item \okt{vpMatrix robot.camJacobian()}: get the Jacobian between the camera velocity screw in its own frame and the robot command $\u$. This Jacobian is to be multiplied
 ...
 \end{itemize}
\item \okt{robot.sendVelocity(vpColVector)}: send a $\u$ velocity. Wheel velocity limits will be applied before actually sending it to the simulator.
\end{itemize}
...
\begin{itemize}
 \item \okt{qp.solveQP ( Q, r, A, b, C, d, x)} for a classical QP with equality and inequality constraints
 \item  \okt{qp.solveQPe ( Q, r, A, b, x)} with only equality constraints
 \item  \okt{qp.solveQPi ( Q, r, C, d, x)} with only inequality constraints
\end{itemize}
